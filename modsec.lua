local ok, ffi = pcall(require,"cffi")
if not ok then
	return nil
end
...
ffi.cdef[[
        ...
        int msc_add_request_header(Transaction *transaction, const char *key,
                                    const char *value);
...
if not ok then
	ok, clib = pcall(ffi.load,"/usr/local/modsecurity/lib/libmodsecurity.so",true)
        if not ok then
            print("Failed to open modsecurity library")
            return nil
        end
end
...
function _M.init(rules_file,rules_remote,rules_inline,remote_key)
    ...
    if type(rules_file) == "string" then
        ...
        if res < 0 then
            print("Failed to load rules", ffi.string(err[0]))
    ...
    print("Loaded #"..res.." rules from file")
